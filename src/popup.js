...
class PopupUI {
  ...
  async loadData() {
    ...
    document.getElementById('local-url').value =
      this.settings.localLlmUrl || 'http://localhost:11434'
  ...
  updateLLMSettings() {
    ...
    document.getElementById('claude-settings').style.display =
    ...
    this.duplicateSelect = new TomSelect(select, {
      ...
      onChange: async (value) => {
        ...
        const labels = {
          none: 'disabled',
          log: 'logging only',
          archive: 'enabled with auto-archive',
        }
  ...
  async saveSettings() {
    ...
    try {
      ...
      try {
        ...
        this.showStatus(
          '✓ Settings saved! (Could not test connection - will verify when classifying)',
          'success'
        )
      }
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          this.showStatus(
            `✓ Credentials saved and verified! (${data.length} subscriptions)`,
            'success'
          )
        } else if (response.status === 401) {
          this.showStatus(
            '⚠️ Credentials saved but may be incorrect. Please verify at feedbin.com',
            'error'
          )
        } else {
        ...
        this.showStatus(
          '✓ Credentials saved! (Could not verify - will test when you use the extension)',
          'success'
        )
      }
  ...
  async renderAllTags() {
    ...
    if (allTags.length === 0) {
      tagsListEl.innerHTML =
        '<span style="color: #9ca3af; font-size: 13px;">No tags yet - classify some entries to get started</span>'
      return
    }
  ...
  async renderStats() {
    ...
    if (totalEntries === 0) {
      statsEl.innerHTML =
        'No entries classified yet. Click "🤖 Classify" in the sidebar to get started.'
      return
    }
    ...
    const statsHtml = `
      ...
      <div style="font-size: 12px; margin-top: 8px;">
        ${Object.entries(tagCounts)
          ...
          .map(
            ([tag, count]) =>
              `<div style="margin: 4px 0;"><span class="feed-tag" style="font-size: 11px;">${tag}</span> × ${count}</div>`
          )
          .join('')}
      </div>
    `
